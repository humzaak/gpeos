...
- (void)processMapLayers
{
    ...
    for (int i = 0, j=self.mapView.mapLayers.count-1; j >= 0 ;j--)
    {     
        //get the agsLayer from the mapLayers array 
        AGSLayer *agsLayer = [self.mapView.mapLayers objectAtIndex:j];
     //  NSLog(@"%@,%d,%d",agsLayer.name,j,i);
        //check to see if the agsLayer exists in the toc or not.
        //|| [agsLayer isKindOfClass:[AGSGraphicsLayer class]] || [agsLayer isKindOfClass:[AGSSketchGraphicsLayer class]]
        if(!([self.mapViewLevelLayerInfo containsChildWithLayerName:agsLayer.name]|| [agsLayer isKindOfClass:[AGSSketchGraphicsLayer class]] ))
        {
